...
abstract class BaseUserGroup extends BaseObject implements Persistent
{
    ...
    public function ensureConsistency()
    {
        ...
        if ($this->aGroup !== null && $this->groupid !== $this->aGroup->getGroupid()) {
            $this->aGroup = null;
        }
    } // ensureConsistency
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(UserGroupPeer::USERID)) {
            $modifiedColumns[':p' . $index++]  = '`UserId`';
        }
        if ($this->isColumnModified(UserGroupPeer::GROUPID)) {
            $modifiedColumns[':p' . $index++]  = '`GroupId`';
        }

        $sql = sprintf(
            'INSERT INTO `UserGroup` (%s) VALUES (%s)',
    ...
    {
        if ($this->aGroup === null && ($this->groupid !== null) && $doQuery) {
